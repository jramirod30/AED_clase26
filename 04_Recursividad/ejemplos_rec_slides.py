...
def contar_iguales1[T](lista: list[T], a: T, i: int = 0) -> int:
    if i < len(lista):
...
def buscar_elem[T](lista: list[T], elem: T, i: int = 0) -> int:
    if i == len(lista):
        return -1
    elif lista[i] == elem:
        return i
...
def binary_search[T](lista: list[T], elem: T) -> int:
...
def merge_lists[T](lista1: list[T], lista2: list[T]) -> list[T]:
    i: int = 0
    j: int = 0
    result: list[T] = []
...
def mergesort[T](lista: list[T]) -> list[T]:
    if len(lista) > 1:
